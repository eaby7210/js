...
var count=4;
 ...
 if(amount && date && note && selected)
 {
    ...
    update_sum();
    console.log(coumt);
    document.getElementsByClassName("add-new-expense")[0].getElementsByClassName.dispay="none";
...
function add_category(){
    var new_category_value=document.getElementdByName("new-category")[0].value;
    if (new_category_value){
        var dropdown =document.getElementsByName("category")[0];
        var availabe_categories= document.getElementsByClassName("available-categories")[0];
        ...
        new_category_span.innerHTML=`<span>${new_category_value}<span class="sum"> -${expense_per_category[count]}R.s spent</span></span>;`
        drowndown.appendChild(new_category_span);
        document.getElementsByClassName("add-category")[0].computedStyleMap.dispay= none;
    }
}
function add_category_page(){
    if(dicument.getElementsByClassName("add-category")[0].style.display=="block"){
